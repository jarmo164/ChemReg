...
- [ ] 1.1 Rewrite the high-level architecture section so it matches the real Spring Boot + React stack.
- [ ] 1.2 Split the broad development document into **pilot-ready MVP**, **post-MVP**, and **enterprise-only** scope.
- [ ] 1.3 Decide which currently documented integrations are mandatory now vs deferred: S3/MinIO, search, notifications, SSO, MFA, compliance imports.
- [ ] 1.4 Define the release bar for “ready”: required flows, required tests, required infra, required documentation.

## 2. Authentication and access control
- [ ] 2.1 Replace the current simplified frontend session model with real access/refresh token handling.
- [ ] 2.2 Enforce authorization consistently in backend controllers/services, not only in UI routing.
- [ ] 2.3 Implement role matrix coverage for Org Admin, EHS Manager, Site Manager, User, Auditor, and Supplier where in MVP scope.
- [ ] 2.4 Define tenant/site/location scoping rules and test them against direct-object access attempts.

## 3. SDS and chemical registry core
- [ ] 3.1 Implement SDS document entities, file storage contract, upload flow, and version management.
- [ ] 3.2 Expose live SDS APIs for list/detail/create/update/upload/download/preview.
- [ ] 3.3 Replace seeded/demo SDS frontend behavior with live API-backed flows and proper empty/error/loading states.
- [ ] 3.4 Expand chemical product management to include the metadata needed by SDS, labels, inventory, and risk workflows.
- [ ] 3.5 Define the minimum searchable/filterable fields needed for MVP even if advanced full-text search is deferred.
...
- [ ] 6.3 Define which reports are truly required for MVP: inventory export, SDS coverage, risk register, manifest.
...
- [ ] 7.2 Add frontend tests for login/session handling and critical module happy paths.
- [ ] 7.3 Add at least one E2E suite for login -> SDS/product -> inventory -> risk happy path.
- [ ] 7.4 Update CI so tests and builds are mandatory gates before deploy.
- [ ] 7.5 Add environment setup, secrets, backup/recovery, and deployment/runbook documentation.
- [ ] 7.6 Verify staging or production-like deployment with a clean environment rehearsal.

## 8. Verification
- [ ] 8.1 Reconcile README, wiki, and development documentation so they tell the same truth.
